...
public class Player {
    ...
    // Constructor
    public Player(int startX, int startY, BufferedImage walkingSpriteSheet, BufferedImage idleSpriteSheet, int tileSize) {
        this.x = startX;
        this.y = startY;
    ...
    public void update(boolean up, boolean down, boolean left, boolean right, Point cursor) {
        ...
        if (cursor != null) { // Ensure cursor is not null
            ...
            if (cursor.getX() < x + tileSize / 2.0) { // Cursor is left of player center
                facingRight = false;
